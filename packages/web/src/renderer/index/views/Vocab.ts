...
export default () => {
    ...
    function parseJieba(s: string) {
        if (s.length > 0) {
            isLoading = true;
            $.post("/api/jieba/", { entry: s }).then((res) => {
    ...
    function parseVocab(vList: string[]) {
        isLoading = true;
        $.post("/api/vocab/match", { entries: vList }).then((res) => {
            i = 0;
            vocabList = res;
            isLoading = false;
            m.redraw();
        ...
        view() {
            ...
            if (currentVocab === undefined) {
                ...
                if (current.simplified !== currentVocab.simplified) {
                    ...
                    $.post("/api/sentence/", { entry: currentVocab.simplified }).then((res) => {
            ...
            return m(".row.mt-3", [
                ...
                m(".row.mt-3.full-width", [
                    m(".col-md-6.text-center", [
                        m("input.col-md-6.vocab-display.vocab.zh-contextmenu", {
                            onkeypress(e: any) {
